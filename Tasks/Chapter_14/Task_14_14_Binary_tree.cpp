...
#include "Task_14_14_Binary_tree.h"
#endif
	...
	:l{ ll }, r{ rr }
{
	...
	{
		v_labels.push_back(new Text(Point(point(i).x, point(i).y), ""));
		if (i < pow(2, l - 1) - 1)
		{
			switch (line_type)
			{
...
void Binary_tree::set_label(std::string text, std::string p)
{
	int n = 0;
	for (int i = 0; i < p.size(); ++i)
	{
		...
		case 'r':
			...
			break;
		default:
			error("Wrong symbol for label.");
			break;
		}
	}
	v_labels[n].set_label(text);
}
