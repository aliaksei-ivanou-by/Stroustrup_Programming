#ifndef TASK_18_12_STDAFX
#include "Task_18_12_stdafx.h"
#include "Task_18_12_HuntTheWumpus.h"
#endif
...
void game_hunt_the_wumpus()
{
	while (true)
	{
		...
		while (true)
		{
			...
			while (std::cin >> ch && !(ch == 'm' || ch == 's' || ch == 'M' || ch == 'S'))
			{
				std::cout << "Shoot or move (s/m)?\n";
			}
			if (ch == 'm' || ch == 'M') // move
			{
				while (true)
				{
					...
					if (cave.player_move(id))
					{
						break;
					}
				}
				if (cave.get_player_location() == cave.get_wumpus_location())
				{
					std::cout << "Wumpus!\n";
					cave.wumpus_move();
					if (cave.get_player_location() == cave.get_wumpus_location())
					{
						std::cout << "Wumpus killed you! You lose!\n";
						break;
					}
				}
				while (cave.get_player_location()->has_bat)
				{
					std::cout << "The bat threw you into another room!\n";
					cave.bat_move();
				}
				if (cave.get_player_location()->has_pit)
				{
					std::cout << "You fell in pit! You lose!\n";
					break;
				}
			}
			else // shoot
			{
				States state = cave.arrow_shoot();
				switch (state)
				{
				case States::player_dead:
				{
					std::cout << "Arrowed kill you! You lose!\n";
					break;
				}
				case States::no_arrows:
				{
					std::cout << "No arrows! You lose\n";
					break;
				}
				case States::wumpus_dead:
				{
					std::cout << "You killed Wumpus! You win!\n";
					break;
				}
				default:
				{
