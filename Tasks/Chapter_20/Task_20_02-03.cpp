...
const double* high(double* first, double* last)
{
	double h = DBL_MIN;
...
const double* high(double* myArray, size_t size)
{
	if (size == 0)
	{
		return nullptr;
	}
	const double* high = &myArray[0];
...
const double* high(const std::vector<double>& myVector)
{
	if (myVector.size() == 0)
	{
		return nullptr;
	}
	const double* high = &myVector[0];
...
try
{
	...
	const double* jack_high_ptr = high(&jack_data[0], &jack_data[0] + jack_count);

	const double* jack_high_arr = high(jack_data.get(), jack_count);
	...
	const double* jack_high_vec = high(jack_vec);

	// Jill Data
	std::unique_ptr<std::vector<double>> jill_data = get_from_jill();

	std::vector<double>& jill_vec = *jill_data;
	const double* jill_high_ptr = high(&jill_vec[0], &jill_vec[0] + jill_vec.size());
	...
	const double* jill_high_arr = high(jill_arr.get(), (*jill_data).size());

	const double* jill_high_vec = high(*jill_data);
