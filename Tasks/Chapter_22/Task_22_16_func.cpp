#ifndef TASK_22_16_STDAFX
#include "Task_22_16_stdafx.h"
#include "Task_22_16_func.h"
#endif
...
Arrow::Arrow(const Point& tail, const Point& tip)
{
	add(tail);
	add(tip);
}

Arrow::Arrow(const Point& tail, const Point& tip, const Color& line_color)
{
	add(tail);
	add(tip);
...
void Arrow::set_size(const int& s)
{
	size = s;
}
...
ProgrammingLanguages::ProgrammingLanguages(const int& yearToAdd, const std::vector<std::string>& ancestorsToAdd) :
	year{ yearToAdd },
	ancestors{ ancestorsToAdd }
{}

int ProgrammingLanguages::getYear() const
{
	return year;
}

std::vector<std::string> ProgrammingLanguages::getAncestors() const
{
	return ancestors;
}
