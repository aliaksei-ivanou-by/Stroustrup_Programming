...
void Graph_lib::MyHist::set_bar_label(int i)
{
	labels[i].set_label(std::to_string(static_cast<int>(get_idata(i))));
}
...
void Graph_lib::MyHist::draw_lines() const
{
	...
	Axis x(Axis::x, point(0), x_size, 0, "x");
	x.label.move(3 * y_size / 4, 0);
	Axis y(Axis::y, point(0), y_size, 0, "y");
	x.draw_lines();
	y.draw_lines();
}
